...
export async function handleDevCallback(bot2, chatId, userId, data) {
  ...
  if (data === "dev_users") {
    await sendDevUsers2(bot2, chatId, userId);
    return true;
  }
  if (data === "dev_errors") {
...
// ══════════════════════════════════════════════════════════════════════════════
// 3 — sendDevUsers2 — قائمة المستخدمين مع تنقل
// ══════════════════════════════════════════════════════════════════════════════
export async function sendDevUsers2(bot2, chatId, userId) {
  ...
  await bot2.sendMessage(
    ...
    { parse_mode: "Markdown", reply_markup: devUsersKeyboard(sorted) }
  );
}
