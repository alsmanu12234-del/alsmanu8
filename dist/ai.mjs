...
export async function handleAiCallback2(bot2, chatId, userId, data) {
  ...
  if (data === "ai_select_model") {
    const { getAvailableModels } = await Promise.resolve().then(() => (init_ai_manager(), ai_manager_exports));
    const models = getAvailableModels();
    const rows = models.map((m) => [{ text: m.label, callback_data: `ai_model_${m.id}` }]);
    rows.push([{ text: "\u{1F519} \u0631\u062C\u0648\u0639", callback_data: "menu_ai" }]);
    await bot2.sendMessage(chatId, "\u{1F9E0} *\u0627\u062E\u062A\u0631 \u0646\u0645\u0648\u0630\u062C \u0627\u0644\u0630\u0643\u0627\u0621 \u0627\u0644\u0627\u0635\u0637\u0646\u0627\u0639\u064A:*", { parse_mode: "Markdown", reply_markup: { inline_keyboard: rows } });
    return true;
  }
  ...
  if (data === "ai_remove_key") {
    const { removeNvidiaKey } = await Promise.resolve().then(() => (init_ai_manager(), ai_manager_exports));
    removeNvidiaKey(userId);
